...
function getTranscode($id, $streamnumber = null)
{
    ...
    $endofffmpeg .= ' -hls_flags delete_segments -hls_time 3 -hls_list_size 6 -hls_allow_cache 1 -hls_delete_threshold 6';
    $endofffmpeg .= ' /opt/streamtool/app/www/' . $setting->hlsfolder . '/' . $stream->id . '_.m3u8  > /dev/null 2>/dev/null & echo $! ';
    if ($trans) {
        $ffmpeg .= ' -y -thread_queue_size 512 -loglevel error -fflags nobuffer -flags low_delay -fflags +genpts -strict experimental -reconnect 1  -reconnect_streamed-reconnect_delay_max 2 -err_detect ignore_err';
